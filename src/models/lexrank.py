...
Supports TF-IDF, neural embeddings, or hybrid similarity modes.
"""
...
class LexRankSummarizer:
    ...
    def _compute_centrality(
        ...
        """
        Compute eigenvector centrality using power iteration with optional position bias.

        Args:
            similarity_matrix: NxN sentence similarity matrix.
            original_positions: Original sentence positions for position bias.
        ...
        for _ in range(self.max_iterations):
            ...
            # Normalize (handle zero norm case)
            norm = np.linalg.norm(new_scores, ord=1)
            if norm > 0:
                new_scores /= norm
            else:
                # Fallback to uniform if scores are zero
                new_scores = np.ones(n) / n
        ...
        if self.position_bias > 0 and original_positions is not None:
            ...
            # Combine scores
            scores = (1 - self.position_bias) * scores + self.position_bias * position_scores
    ...
    def summarize(
        ...
        # Build similarity matrix (uses embeddings, TF-IDF, or hybrid based on mode)
        similarity_matrix = self._build_similarity_matrix(valid_processed, valid_original)

        # Compute centrality scores with position bias
        original_positions = list(range(len(valid_indices)))
        scores = self._compute_centrality(similarity_matrix, original_positions)

        # Get sentence vectors for MMR (uses embeddings or TF-IDF based on mode)
        ...
        if target_words is not None:
            ...
            for idx in ranked_indices:
                ...
                if current_words + sent_words <= target_words * 1.2:
